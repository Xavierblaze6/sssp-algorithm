...
Output:
    - experiment_results.csv: Comprehensive results for all experiments
    - graphs/: Generated graph files (if save_graphs=True)
"""
...
def run_single_experiment(
    ...
    """
    Run a single experiment on a generated graph.
    
    Args:
        graph_size: Number of vertices (n)
        edge_multiplier: Number of edges = n * edge_multiplier
        source: Source vertex
        seed: Random seed for reproducibility
    
    Returns:
        List of result dictionaries (one per algorithm: dijkstra, sssp, hybrid)
    """
    ...
    # Extract results into rows for CSV
    rows = []
    ...
    # SSSP row (handle possible timeout where stats may be missing)
...
def run_all_experiments(
    ...
    edge_multiplier: float = 5.0,
    num_trials: int = 1,
    output_file: str = "experiment_results.csv",
    save_graphs: bool = False
) -> List[Dict[str, Any]]:
    """
    Run experiments on multiple graph sizes.
    
    Args:
        graph_sizes: List of graph sizes to test (default: [50, 100, 500, 1000, 5000, 10000])
        edge_multiplier: Edges per vertex
        num_trials: Number of trials per size (for averaging)
        output_file: CSV output filename
        save_graphs: Whether to save generated graphs to files
    ...
    for size_idx, size in enumerate(graph_sizes):
        ...
        for trial in range(num_trials):
            seed = 42 + size + trial  # Reproducible but different per size/trial
...
def print_summary_table(results: List[Dict[str, Any]]):
    """
    Print a summary table of results grouped by graph size.
    """
    # Group by graph size
    by_size = {}
    for row in results:
...
if __name__ == "__main__":
    # Configuration
    GRAPH_SIZES = [50, 100, 500, 1000, 5000, 10000]
    EDGE_MULTIPLIER = 5.0  # m = n * 5
    NUM_TRIALS = 1  # Increase for statistical significance
    ...
    if len(sys.argv) > 1:
        try:
            ...
            print(f"Usage: python run_experiments.py [sizes]")
    ...
    if results:
        ...
        print(f"\nNext steps:")
        print(f"  1. Analyze results: Open {OUTPUT_FILE} in Excel/Python")
        print(f"  2. Generate plots: python plot_results.py")
